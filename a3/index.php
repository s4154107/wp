...
$carouselSql = "SELECT * FROM pets ORDER BY pet_id DESC LIMIT 4";
$carouselResult = mysqli_query($conn, $carouselSql);

$latestSql = "SELECT p.*, u.username 
              FROM pets p 
    ...
    <div class="carousel-inner">
      <?php $active=true; while($pet=mysqli_fetch_assoc($carouselResult)): ?>
        <div class="carousel-item <?= $active ? 'active' : '' ?>">
          <img src="assets/images/pets/<?= htmlspecialchars($pet['image']) ?>" class="carousel-image" alt="<?= htmlspecialchars($pet['name']) ?>">
    ...
    <?php while($pet=mysqli_fetch_assoc($latestResult)): ?>
      <div class="col-md-6 col-lg-3">
        <div class="card pet-card h-100">
          <img src="assets/images/pets/<?= htmlspecialchars($pet['image']) ?>" class="pet-thumb" alt="<?= htmlspecialchars($pet['name']) ?>">
          <div class="card-body">
            <h5><?= h($pet['name']) ?></h5>
            <p><?= h(substr($pet['description'],0,35)) ?>...</p>
